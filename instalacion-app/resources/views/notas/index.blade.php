            ...
            <div class="col-md-9">
                <div class="card">
                    <div class="card-body">
                        ...
                        <br />
                        <br />
                        <form action="{{ route('notas.index') }}" method="GET" class="flex gap-2 pb-4 mx-16 w-full justify-center items-center">
                            <div>Filtrar por asignatura:</div>
                            <select name="buscador" id="buscador" class="rounded-lg" onchange="this.form.submit()">
                                <option value="">Asignatura</option>
                                @foreach ($asignaturas as $asignatura)
                                    <option value="{{ $asignatura->asignatura }}"
                                        {{ $asignatura->asignatura == request('buscador') ? 'selected' : '' }}>
                            ...
                            <table class="table w-full">
                                <thead class="bg-teal-600 text-white">
                                    <tr>
                                        ...
                                        <th class="w-1/5 text-center py-3 px-4 uppercase font-semibold text-sm">Nota
                                        </th>
                                        <th class="w-1/5 text-center py-3 px-4 uppercase font-semibold text-sm">palabras
                                            clave</th>
                                        <th class="w-1/5 text-center py-3 px-4 uppercase font-semibold text-sm">Acciones
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                        ...
                                        <tr class="{{ $loop->iteration % 2 == 0 ? 'bg-gray-300' : 'bg-gray-100' }}">
                                            ...
                                            <td class="w-1/5 text-center py-3 px-4 uppercase font-semibold text-sm">
                                                <a href="{{ url('/notas/' . $item->id) }}" title="View nota"><button
                                                        class="inline-flex m-2"><svg xmlns="http://www.w3.org/2000/svg"
