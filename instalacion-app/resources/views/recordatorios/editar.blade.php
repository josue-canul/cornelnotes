            ...
            <form action="{{ url('recordatorios/' . $recordatorio->id) }}" method="post"
                ...
                <div class="text-center">
                    ...
                    <textarea type="text" name="contenido" id="contenido"
                        class="text-center w-2/4 h-40 rounded-3xl m-5 bg-white py-3 font-medium text-black outline-none focus:border-blue-500 focus:shadow-md">{{ $recordatorio->contenido }}</textarea><br>
                    <label for="importancia">importancia:</label><br>
                    <select name="importancia" id="importancia" class="rounded-3xl text-black">
                        <option value="muy importante" @if($recordatorio->importancia == 'muy importante') selected @endif>Muy importante</option>
                        <option value="importante" @if($recordatorio->importancia == 'importante') selected @endif>Importante</option>
                        <option value="No importante" @if($recordatorio->importancia == 'No importante') selected @endif>No importante</option>
                    </select>
                    <br><br>
